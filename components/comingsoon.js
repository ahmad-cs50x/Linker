...
const ComingSoon = ({ pageName = 'Coming Soon' }) => {
  // Logic: Split the string at the first space
  // We use max 2 parts so that if there are multiple spaces, 
  // everything after the first word stays together in the second part.
  const [firstWord, ...rest] = pageName.split(' ');
    ...
    <div className="min-h-screen relative overflow-hidden bg-[#fcfcfd]">
      ...
      <main className="pt-40 pb-20">
        <section className="max-w-4xl mx-auto px-6 text-center reveal active">
          ...
          <h1 className="text-6xl md:text-7xl font-bold font-heading leading-none tracking-tighter mb-8">
            {/* First word gets the black color */}
            <span className="text-[#000000]">{firstWord}</span>
            
            {/* The rest gets the accent color. We add a space in the text or margin */}
            {remainingText && (
              <span className="text-[#FF6B35] ml-[0.3em]">{remainingText}</span>
            )}
          </h1>
